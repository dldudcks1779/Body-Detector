           ...
           0 < right_hip.x < 1 and 0 < right_hip.y < 1:
            ...
            if abs(left_shoulder.y - right_shoulder.y) < 0.05 and \
               ...
               0.12 < abs(right_hip.z - right_shoulder.z) < 0.45:
                ...
                cv2.putText(frame, "Good", (int(nose.x * frame.shape[1]) - 20, int(nose.y * frame.shape[0]) - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                cv2.putText(frame, "Good", (int(left_shoulder.x * frame.shape[1]) - 20, int(left_shoulder.y * frame.shape[0]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                cv2.putText(frame, "Good", (int(right_shoulder.x * frame.shape[1]) - 20, int(right_shoulder.y * frame.shape[0]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                
                start_time = time.time()
            else:
                if time.time() - start_time > warning_time:
                    ...
                    if 0.12 < abs(left_hip.z - left_shoulder.z) and 0.12 < abs(right_hip.z - right_shoulder.z):
                        ...
                        cv2.putText(frame, "-" + str(b), (int(nose.x * frame.shape[1]) - 20, int(nose.y * frame.shape[0]) - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                        ...
                        if abs(left_shoulder.y - right_shoulder.y) > 0.05:
                            if left_shoulder.y < right_shoulder.y:
                                ...
                                cv2.putText(frame, str(a), (int(left_shoulder.x * frame.shape[1]) - 20, int(left_shoulder.y * frame.shape[0]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                                cv2.putText(frame, "-" + str(a), (int(right_shoulder.x * frame.shape[1]) - 20, int(right_shoulder.y * frame.shape[0]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                            else:
                                a = int(abs((left_shoulder.y - right_shoulder.y) * 100))
                                cv2.putText(frame, "-" + str(a), (int(left_shoulder.x * frame.shape[1]) - 20, int(left_shoulder.y * frame.shape[0]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                                cv2.putText(frame, str(a), (int(right_shoulder.x * frame.shape[1]) - 20, int(right_shoulder.y * frame.shape[0]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                    # 구부렸을 때
                        ...
                        cv2.putText(frame, str(b), (int(nose.x * frame.shape[1]) - 20, int(nose.y * frame.shape[0]) - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)                    
